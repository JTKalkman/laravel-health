...
class DiskSpaceCheckTest extends TestCase
{
    ...
    {
        $check = new DiskSpaceCheck('/var');
        $result = $check->run();
 
        $this->assertEquals('Disk space /var', $result->name);
    }

    public function test_accepts_custom_name(): void
    {
        $check = new DiskSpaceCheck(path: '/', name: 'Primary disk');
        $result = $check->run();
    ...
    {
        $check = new DiskSpaceCheck('/path/to/check');
        $result = $check->run();
    ...
    public function test_returns_ok_when_below_warning_threshold(): void
    {
        ...
        $check = new DiskSpaceCheck(path: '/', warningThreshold: 99, errorThreshold: 100);
        $result = $check->run();
    ...
    public function test_returns_warning_when_above_warning_threshold(): void
    {
        ...
        $check = new DiskSpaceCheck(path: '/', warningThreshold: 0, errorThreshold: 100);
        $result = $check->run();
    ...
    {
        $check = new DiskSpaceCheck('/this/path/does/absolutely/not/exist');
        $result = $check->run();
